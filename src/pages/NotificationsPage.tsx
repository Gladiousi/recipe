...
const NotificationsPage = () => {
  ...
  const handleReject = async (id: number) => {
    setProcessingId(id);
    try {
      await groupsAPI.rejectInvitation(id);
      setInvitations(prev => prev.filter(inv => inv.id !== id));
    } catch (error) {
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm text-muted-foreground mb-3">
                      <span className="font-medium text-foreground">{invitation.invited_by.username}</span>
                      {' '}пригласил вас в группу{' '}
                      <span className="font-medium text-foreground">{invitation.group.name}</span>
                    </p>

                    {/* Inviter */}
                    <div className="flex items-center gap-2 mb-4">
                      <Avatar className="h-8 w-8">
                        <AvatarImage src={invitation.invited_by.avatar} />
